...
RAW_DIR = Path("data/raw/corruptmnist")
PROCESSED_DIR = Path("data/processed")


def normalize(images: torch.Tensor, mean: torch.Tensor, std: torch.Tensor) -> torch.Tensor:
    """Normalize images using provided mean/std."""
    return (images - mean) / (std + 1e-8)



def preprocess_data(raw_dir: str = str(RAW_DIR), processed_dir: str = str(PROCESSED_DIR)) -> None:
